...
Level* Level::create(uint64_t width, uint64_t height)
{
	...
	level->mMovementMap = level->mMapData + ( level->mSize * 0 );
	level->mTileMap     = level->mMapData + ( level->mSize * 1 );
	level->mOverTileMap = level->mMapData + ( level->mSize * 2 );
	level->mOverMap     = level->mMapData + ( level->mSize * 3 );
	level->mCharMap     = level->mMapData + ( level->mSize * 4 );
